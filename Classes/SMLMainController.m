...
- (void)checkForUpdateInSeparateThread
{
	...
	BOOL success = SCNetworkCheckReachabilityByName("smultron.sourceforge.net", &status); 
	BOOL connected = success && (status & kSCNetworkFlagsReachable) && !(status & kSCNetworkFlagsConnectionRequired); 
	if (connected) {
		NSDictionary *dictionary = [NSDictionary dictionaryWithContentsOfURL:[NSURL URLWithString:@"http://smultron.sourceforge.net/checkForUpdate.plist"]];
		if (dictionary) {
