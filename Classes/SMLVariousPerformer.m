...
- (void)checkIfDocumentsHaveBeenUpdatedByAnotherApplication
{
	...
	for (id item in array) {
		...
		NSDictionary *attributes = [[NSFileManager defaultManager] fileAttributesAtPath:[item valueForKey:@"path"] traverseLink:YES];
		...
		if (![[[item valueForKey:@"fileAttributes"] fileModificationDate] isEqualToDate:[attributes fileModificationDate]]) {
			if ([[SMLDefaults valueForKey:@"UpdateDocumentAutomaticallyWithoutWarning"] boolValue] == YES) {
				...
				[item setValue:[[NSFileManager defaultManager] fileAttributesAtPath:[item valueForKey:@"path"] traverseLink:YES] forKey:@"fileAttributes"];
			} else {
...
- (void)sheetDidFinish:(NSWindow *)sheet returnCode:(NSInteger)returnCode contextInfo:(void *)contextInfo;
{
	...
	if (returnCode == NSAlertDefaultReturn) {
		...
		[document setValue:[[NSFileManager defaultManager] fileAttributesAtPath:[document valueForKey:@"path"] traverseLink:YES] forKey:@"fileAttributes"];
	}
}
